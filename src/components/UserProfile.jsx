...
function UserProfile({ onBack, profile }) {
  ...
  useEffect(() => {
    async function loadRepositories() {
      ...
      try {
        setRepositories(await getUserRepositories(profile.login))
      } catch (requestError) {
  ...
  return (
    <section className="mx-auto mt-16 max-w-3xl text-left">
      ...
      <div className="mt-8"><h2 className="text-xl font-semibold text-[#eef2ff]">Repositories</h2>{loadingRepositories && <p className="mt-4 text-[#f59e0b]">Loading repositories...</p>}{repositoriesError && <p className="mt-4 text-rose-300" role="alert">{repositoriesError}</p>}{!loadingRepositories && !repositoriesError && repositories.length === 0 && <p className="mt-4 text-[#a8b5d8]">This user has no public repositories.</p>}{!loadingRepositories && !repositoriesError && repositories.length > 0 && <div className="mt-4 grid gap-4 sm:grid-cols-2">{repositories.map((repository) => <RepositoryResultCard key={repository.id} repository={repository} />)}</div>}</div>
    </section>
  )
}
